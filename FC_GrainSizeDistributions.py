...
plt.semilogx(df.Mesh[:-2],np.cumsum(df.Sediment1[:-2]),'.:',label='Sample 1',linewidth=lnwidth,markersize=mrkrsize)
plt.semilogx(df.Mesh[:-2],np.cumsum(df.Sediment3[:-2]),'.:',label='Sample 2',linewidth=lnwidth,markersize=mrkrsize)
plt.semilogx(df.Mesh[:-2],np.cumsum(df.Sediment4[:-2]),'.:',label='Sample 3',linewidth=lnwidth,markersize=mrkrsize)
plt.semilogx(df.Mesh[:-2],np.cumsum(df.Sediment5[:-2]),'.:',label='Sample 4',linewidth=lnwidth,markersize=mrkrsize)
plt.semilogx(df.Mesh[:-2],np.cumsum(df.Sediment6[:-2]),'.:',label='Sample 5',linewidth=lnwidth,markersize=mrkrsize)
...
#plt.show()
plt.savefig('/Users/mistral/Documents/CUBoulder/Science/Sulzer/data/grainsizedist.png',bbox_extra_artists=(lgd,), bbox_inches='tight',pad_inches=0.4)
